   ...
   Leaf<T> Find(T key) 
{
    ...
    else if(m_this_node[::Last] < key && m_next_b) return m_next_b->Find(key);
    else if(m_this_node[::Middle].m_isValid)
    {
        if (m_this_node[::Middle] == key) return m_this_node[::Middle];
        else if (m_this_node[::Middle] < key && m_next_m) return m_next_m->Find(key);
    }
    else if (m_next_l) return m_next_l->Find(key);
...
int key = 3 ;
auto temp = Btree.Find(key);
std::cout << temp.get_value()<<temp.is_valid() << std::endl ;
