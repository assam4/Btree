...
public:
    ...
    void redistribution(Leaf<T> Middle)
    {
        ...
        else if(m_values_count == Binar) fill_the_middle() ;
        count_update();
    }
    void divide()
{
    ...
    if (m_is_node && m_next_m)
    {
        ...
        else if (m_next_b->m_values_count == Binar)
        {

            left->m_next_l = std::exchange(m_next_l, nullptr);
            left->m_next_b = std::exchange(m_next_m, nullptr);
            right->m_next_l = std::exchange(m_next_b, nullptr);
            auto temp = std::exchange(right->m_next_l->m_this_node[Last], Leaf<T>());  
            right->m_next_l->sorting() ;
            right->m_next_b->push_here(temp); 
            right->m_next_l->count_update();
        }
        else
        {
